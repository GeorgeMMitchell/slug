  ...
  std::basic_string_view<Char, CharTraits> fmt, Args&&... args)
{
  ...
  auto&& make_fmtargs = [](auto&&... t_args) {
    ...
    else return fmt::make_wformat_args(t_args...);
  };

  auto&& fmtargs = make_fmtargs(args...);
  auto&& membuf  = fmt_membuf_t{alloc};

  fmt::vformat_to(std::back_inserter(membuf), fmt, fmtargs);

  return {membuf.data(), membuf.size(), alloc};
}
...
struct basic_message_format_base {
  ...
  get_severity_literal(severity_t severity) noexcept
  {
    ...
    constexpr char_t m_debug_chars[]   = {'d', 'e', 'b', 'u', 'g', 0};
    constexpr char_t m_trace_chars[]   = {'t', 'r', 'a', 'c', 'e', 0};
...
struct basic_logger final {
  ...
  log(severity_t severity, std_string_view_t fmt, Args&&... args)
  {
    ...
    auto&& msgdata = message_data_t{
      std::move(msgstr), severity, m_start_time, m_char_allocator};
